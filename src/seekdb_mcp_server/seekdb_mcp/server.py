...
def create_collection(
    ...
    Args:
        ...
        distance: The distance metric for vector similarity. Options: 'cosine', 'l2', 'ip' (inner product). Default is 'cosine'.
        use_embedding_function: Whether to use the default embedding function. If True, uses DefaultEmbeddingFunction (384 dimensions). If False, vectors must be provided manually. Default is True.

    Returns:
        A JSON string indicating success or error.
    """
    logger.info(f"Calling tool: create_collection with arguments: collection_name={collection_name}, dimension={dimension}, distance={distance}, use_embedding_function={use_embedding_function}")
    ...
    try:
        ...
        result["success"] = True
        result["message"] = f"Collection '{collection_name}' created successfully with dimension={dimension}, distance={distance}"
    except Exception as e:
        result["error"] = f"[Exception]: {e}"
...
def add_data_to_collection(
    ...
    Args:
        ...
        embeddings: A list of pre-computed vectors. Each vector should match the dimension of the collection.
                    Optional if documents are provided and the collection has an embedding_function.
    ...
    """
    logger.info(f"Calling tool: add_data_to_collection with arguments: collection_name={collection_name}, ids={ids}")
    ...
    try:
        ...
        result["success"] = True
        result["message"] = f"Successfully added {len(ids)} item(s) to collection '{collection_name}'"
    except Exception as e:
        result["error"] = f"[Exception]: {e}"
...
def query_collection(
    ...
    """
    logger.info(f"Calling tool: query_collection with arguments: collection_name={collection_name}, n_results={n_results}")
    ...
    try:
        ...
        result["message"] = f"Query returned {len(formatted_results['ids'][0]) if formatted_results['ids'] else 0} result(s)"
    except Exception as e:
        result["error"] = f"[Exception]: {e}"
...
def delete_collection(collection_name: str) -> str:
    ...
    Returns:
        A JSON string indicating success or error.
    """
    logger.info(f"Calling tool: delete_collection with arguments: collection_name={collection_name}")
    result = {"collection_name": collection_name, "success": False, "error": None}
...
def full_text_search(
    ...
    Args:
        ...
        return_score: Whether to return relevance scores. Default is True.
    ...
    """
    logger.info(f"Calling tool: full_text_search with arguments: table_name={table_name}, column_name={column_name}, search_expr={search_expr}, mode={mode}")
    ...
    try:
        ...
        if result["success"]:
            result["message"] = f"Full-text search returned {len(result['data']) if result['data'] else 0} result(s)"
    except Exception as e:
        result["error"] = f"[Exception]: {e}"
...
def hybrid_search(
    ...
    knn_query_embeddings: Optional[list[list[float]]] = None,
    ...
    Args:
        ...
        knn_query_embeddings: Pre-computed query vectors for similarity search.
                              Required if knn_query_texts is not provided.
    ...
    Examples:
        ...
        - Hybrid search with embeddings:
          hybrid_search("my_collection",
                        fulltext_search_keyword="database",
                        knn_query_embeddings=[[0.1, 0.2, 0.3]],
                        n_results=5)
    """
    ...
    try:
        ...
        if knn_query_embeddings:
            knn_config["query_embeddings"] = knn_query_embeddings
        if knn_where:
        ...
        result["success"] = True
        result["data"] = formatted_results
        result["message"] = f"Hybrid search returned {len(formatted_results['ids'][0]) if formatted_results['ids'] else 0} result(s)"
    except Exception as e:
        result["error"] = f"[Exception]: {e}"
...
def create_ai_model(
    ...
    """
    logger.info(f"Calling tool: create_ai_model with arguments: model_name={model_name}, model_type={model_type}")
    ...
    try:
        ...
        if result["success"]:
            result["message"] = f"AI model '{model_name}' created successfully with type={model_type}, provider_model={provider_model_name}"
    except Exception as e:
        result["error"] = f"[Exception]: {e}"
...
def create_ai_model_endpoint(
    ...
    Examples:
        ...
        - Create a completion endpoint:
          create_ai_model_endpoint("ob_complete_endpoint", "ob_complete",
                                   ...
                                   "sk-xxxxx", "siliconflow")
    """
    logger.info(f"Calling tool: create_ai_model_endpoint with arguments: endpoint_name={endpoint_name}, ai_model_name={ai_model_name}")
    ...
    try:
        ...
        logger.info(f"Executing SQL: CALL DBMS_AI_SERVICE.CREATE_AI_MODEL_ENDPOINT('{endpoint_name}', '...')")
        ...
        if result["success"]:
            result["message"] = f"AI model endpoint '{endpoint_name}' created successfully for model '{ai_model_name}'"
    except Exception as e:
        result["error"] = f"[Exception]: {e}"
...
def drop_ai_model_endpoint(endpoint_name: str) -> str:
    ...
    """
    logger.info(f"Calling tool: drop_ai_model_endpoint with arguments: endpoint_name={endpoint_name}")
    result = {"endpoint_name": endpoint_name, "success": False, "error": None}
...
def ai_complete(
    ...
    try:
        ...
        if template_args:
            ...
            args_str = ", ".join([f"'{arg.replace(chr(39), chr(39)+chr(39))}'" for arg in template_args])
...
def ai_embed(
    ...
    """
    Convert text to vector embeddings using AI_EMBED function in seekdb.

    AI_EMBED calls a registered embedding model to convert text data into vector representations.
    This is a fundamental step in vector retrieval, converting text data into high-dimensional 
    vector representations for similarity calculations.

    Args:
        model_name: The name of the registered embedding model. Example: "ob_embed"
        input_text: The text you want to convert into a vector. Example: "Hello world"
        dim: Optional. Specifies the output dimension of the vector. 
             Some model providers support configuring this value.
             If not specified, uses the model's default dimension.

    Returns:
        A JSON string containing the vector embedding result or error.
        The embedding is returned as a vector array, e.g., [0.1, 0.2, 0.3, ...]

    Examples:
        - Embed a single text:
          ai_embed("ob_embed", "Hello world")
          Returns: {"success": true, "embedding": "[0.1, 0.2, 0.3, ...]"}
        
        - Embed with specific dimension:
          ai_embed("ob_embed", "Hello world", dim=512)
        
        - Embed for semantic search:
          ai_embed("ob_embed", "What is machine learning?")
        
        - Embed document content:
          ai_embed("ob_embed", "SeekDB is a powerful database system that supports vector retrieval.")
    """
    logger.info(f"Calling tool: ai_embed with arguments: model_name={model_name}")
    result = {"model_name": model_name, "success": False, "embedding": None, "error": None}
    
    try:
        # Escape single quotes in input_text
        escaped_input = input_text.replace("'", "''")
        
        if dim is not None:
            # With dimension parameter
            sql = f"SELECT AI_EMBED('{model_name}', '{escaped_input}', {dim}) AS embedding"
        else:
            # Without dimension parameter
            sql = f"SELECT AI_EMBED('{model_name}', '{escaped_input}') AS embedding"
        
        logger.info(f"Executing AI_EMBED query")
        ...
        if result["success"] and sql_result.get("data"):
            # Extract the embedding from the query result
            result["embedding"] = sql_result["data"][0][0] if sql_result["data"] else None
            result["message"] = "Text successfully converted to vector embedding"
    except Exception as e:
        result["error"] = f"[Exception]: {e}"
        logger.error(f"Failed to execute AI embed: {e}")
...
def create_semantic_index(
    ...
    """
    logger.info(f"Calling tool: create_semantic_index with arguments: table_name={table_name}, column_name={column_name}")
...
WITH (distance={distance}, lib=vsag, type=hnsw, model={model_name}, dim={dimension}, sync_mode={sync_mode})"""
        ...
        if result["success"]:
            result["message"] = f"Semantic index '{index_name}' created successfully on {table_name}.{column_name}"
    except Exception as e:
        result["error"] = f"[Exception]: {e}"
...
def semantic_search(
    ...
    """
    logger.info(f"Calling tool: semantic_search with arguments: table_name={table_name}, query_text={query_text}")
    result = {"table_name": table_name, "success": False, "data": None, "error": None}
...
ORDER BY semantic_distance({column_name}, '{escaped_query}') 
APPROXIMATE LIMIT {limit}"""
        ...
        if result["success"]:
            result["message"] = f"Semantic search returned {len(result['data']) if result['data'] else 0} result(s)"
    except Exception as e:
        result["error"] = f"[Exception]: {e}"
...
APPROXIMATE LIMIT {limit}"""
        ...
        if result["success"]:
            result["message"] = f"Semantic vector search returned {len(result['data']) if result['data'] else 0} result(s)"
    except Exception as e:
        result["error"] = f"[Exception]: {e}"
